<template>
  <div class="max-w-4xl mx-auto p-6">
    ...
    <div class="bg-white p-4 rounded-xl shadow mb-6 flex items-center gap-4">
      ...
      <button
        @click="editProfile"
        class="ml-auto bg-blue-500 text-white px-4 py-2 rounded-xl"
      >
        프로필 수정
      </button>
    </div>
    ...
    <div>
      <div v-if="activeTab === '주문 내역'">
        <div
          ...
          :key="order.id"
          class="p-4 bg-white rounded-xl shadow mb-4"
        >
      ...
      <div v-if="activeTab === '리뷰 관리'">
        ...
        >
          ...
          <button class="text-blue-500 mr-2" @click="editReview(review.id)">
            수정
          </button>
          <button class="text-red-500" @click="deleteReview(review.id)">
            삭제
          </button>
        </div>
      </div>
      ...
      <div v-if="activeTab === '찜 목록'" class="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <button class="mt-2 w-full bg-red-500 text-white py-1 rounded-lg">
            찜 해제
          </button>
        </div>
      </div>
